...
Baseline model + Model med kliniske interactions
```{r}
...
Model med statistiske interaktioner 
```{r}

base_predictors <- c("EDSS", "Relapse24mth", "AgeGroup", "YearAtFirstDiagnosis", "Sex", "BrainT2")
response <- "TreatmentDrugCategorized"

interaction_terms <- c(
  "EDSS:Relapse24mth",
  "EDSS:AgeGroup",
  "EDSS:YearAtFirstDiagnosis",
  "EDSS:Sex",
  "EDSS:BrainT2",
  "Relapse24mth:AgeGroup",
  "Relapse24mth:YearAtFirstDiagnosis",
  "Relapse24mth:BrainT2",
  "AgeGroup:YearAtFirstDiagnosis",
  "AgeGroup:BrainT2",
  "AgeGroup:Sex",
  "YearAtFirstDiagnosis:BrainT2",
  "YearAtFirstDiagnosis:Sex",
  "BrainT2:Sex",
  "Relapse24mth:Sex"
)
...
# --- 1. Baseline model ---
baseline_formula_str <- paste(response, "~", paste(base_predictors, collapse = " + "))
baseline_model <- glm(as.formula(baseline_formula_str), data = all_filtered_imputed, family = binomial())
models[["baseline_model"]] <- baseline_model
model_summary <- rbind(model_summary, data.frame(
  Model = "baseline_model",
  AIC = AIC(baseline_model),
  Deviance = deviance(baseline_model)
))

# --- 2. Clinical model ---
clinical_formula_str <- paste(
  response, "~", paste(base_predictors, collapse = " + "),
  "+ AgeGroup:Sex + Relapse24mth:Sex"
)
clinical_model <- glm(as.formula(clinical_formula_str), data = all_filtered_imputed, family = binomial())
models[["clinical_model"]] <- clinical_model
model_summary <- rbind(model_summary, data.frame(
  Model = "clinical_model",
  AIC = AIC(clinical_model),
  Deviance = deviance(clinical_model)
))

# --- 3. Interaction models (one interaction at a time) ---
for (interaction in interaction_terms) {
  formula_str <- paste(response, "~", paste(base_predictors, collapse = " + "), "+", interaction)
  model_formula <- as.formula(formula_str)
  model_name <- paste0("interaction_", gsub(":", "_", interaction))
  
  model <- glm(model_formula, data = all_filtered_imputed, family = binomial())
...
Da vi finder frem til forskellige interaktioner er gode, vil vi nu kombinere dem og se om det bliver til mere passende modeller.
```{r}
...
# --- Loop over all combinations of size 1 to 4 ---
interaction_combos <- list()

for (k in 1:length(top_interactions)) {
  interaction_combos <- c(interaction_combos, combn(top_interactions, k, simplify = FALSE))
}

for (i in seq_along(interaction_combos)) {
  interactions <- interaction_combos[[i]]

  full_formula_str <- paste(
    response, "~",
    paste(base_predictors, collapse = " + "),
    "+",
    paste(interactions, collapse = " + ")
  )
  
  model_name <- paste0("combo_", i, "_", gsub(":", "_", paste(interactions, collapse = "_")))

  model <- glm(as.formula(full_formula_str), data = all_filtered_imputed, family = binomial())
  models[[model_name]] <- model
...
Nye model med interaktioner 
```{r}
final_model_1 <- glm(TreatmentDrugCategorized ~ 
                                    Sex + Relapse24mth + YearAtFirstDiagnosis + EDSS + BrainT2 + AgeGroup + 
                                    Relapse24mth * YearAtFirstDiagnosis + Relapse24mth * Sex,
                                  data = all_filtered_imputed,
                                  family = binomial)

final_model_2 <- glm(TreatmentDrugCategorized ~ 
                                    Sex + Relapse24mth + YearAtFirstDiagnosis + EDSS + BrainT2 + AgeGroup + 
                                    Relapse24mth * YearAtFirstDiagnosis + EDSS * Relapse24mth + Relapse24mth * Sex,
                                  data = all_filtered_imputed,
                                  family = binomial)


anova(final_model_1, final_model_2, test = "LRT")
...
backward_model <- stepAIC(final_model_2, direction = "backward")

summary(backward_model)
```


Summary af final_model_1
```{r}
...
Indsætter Cut-off værdi ind på grafen 
```{r}
...
confusion_matrix <- confusionMatrix(factor(predicted_class),
                                     factor(all_filtered_imputed$TreatmentDrugCategorized),
                                     positive = "1")
print(confusion_matrix)
...
coords_point <- coords(roc_curve, x = 0.245, input = "threshold", ret = c("specificity", "sensitivity"))
...
points(1 - coords_point["specificity"], coords_point["sensitivity"], col = "red", pch = 19, cex = 1.5)
text(1 - coords_point["specificity"], coords_point["sensitivity"],
     labels = "Model (cut-off = 0.245)", pos = 4, col = "red", cex = 0.9)


text(0.5, 0.2, paste("AUC: ", round(auc_value, 2)), col = "blue", cex = 1.5)
points(0, 1, col = "darkgreen", pch = 19, cex = 1.5)
text(0, 1, "Perfekt Klassifikator", col = "darkgreen", pos = 4, cex = 0.9)
...
results_edss$Significance <- cut(results_edss$p_value, 
...
ggplot(results_edss, aes(x = as.factor(Year), y = as.factor(EDSS_value), fill = Significance)) +
  ...
  scale_fill_manual(values = c("p>0.05 (ikke signifikant)" = "lightgray", 
                               ...
                               "p<0.01" = "steelblue", 
                               "p<0.001" = "darkblue")) +
  labs(title = "Signifikans af EDSS-niveauer over tid",
       x = "År",
       y = "EDSS-niveau",
       fill = "Signifikans") +
  theme_minimal() +
...
results_brainT2$Significance <- cut(results_brainT2$p_value, 
                                    breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                                    labels = c("p<0.001", "p<0.01", "p<0.05", "p>0.05 (ikke signifikant)"))
...
ggplot(results_brainT2, aes(x = as.factor(Year), y = BrainT2_value, fill = Significance)) +
  ...
  scale_fill_manual(values = c("p>0.05 (ikke signifikant)" = "lightgray", 
                               ...
                               "p<0.01" = "steelblue", 
                               "p<0.001" = "darkblue")) +
  labs(title = "Signifikans af BrainT2-niveauer over tid",
       x = "År",
       y = "BrainT2-niveau",
       fill = "Signifikans") +
...
results_agegroup$Significance <- cut(results_agegroup$p_value, 
                                     breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                                     labels = c("p<0.001", "p<0.01", "p<0.05", "p>0.05 (ikke signifikant)"))
...
ggplot(results_agegroup, aes(x = as.factor(Year), y = AgeGroup_value, fill = Significance)) +
  ...
  scale_fill_manual(values = c("p>0.05 (ikke signifikant)" = "lightgray", 
                               ...
                               "p<0.01" = "steelblue", 
                               "p<0.001" = "darkblue")) +
  labs(title = "Signifikans af AgeGroup over tid",
       x = "År",
       y = "Aldersgruppe",
       fill = "Signifikans") +
...
results_sex_relapse$Significance <- cut(results_sex_relapse$p_value, 
                                        breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                                        labels = c("p<0.001", "p<0.01", "p<0.05", "p>0.05 (ikke signifikant)"))
...
ggplot(results_sex_relapse, aes(x = as.factor(Year), y = Variable, fill = Significance)) +
  ...
  scale_fill_manual(values = c("p>0.05 (ikke signifikant)" = "lightgray", 
                               ...
                               "p<0.01" = "steelblue", 
                               "p<0.001" = "darkblue")) +
  labs(title = "Signifikans af Sex og Relapse over tid",
       x = "År",
       y = "Variabel",
       fill = "Signifikans") +
  theme_minimal() +
